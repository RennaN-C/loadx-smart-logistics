...
vi.mock("@react-three/fiber", () => {
  ...
  const state = { camera: { id: "camera" }, gl: { domElement: { id: "canvas" } } };
...
describe("CameraControls", () => {
  ...
  it("aponta a câmera para o alvo recebido", () => {
    render(<CameraControls target={[1, 2, 3]} />);
  ...
  it("NÃO descarta os controles quando o alvo é remontado com os mesmos valores", () => {
    ...
    const { rerender } = render(<CameraControls target={[1, 2, 3]} />);

    rerender(<CameraControls target={[1, 2, 3]} />);
    rerender(<CameraControls target={[1, 2, 3]} />);
  ...
  it("reposiciona sem descartar quando o alvo muda de lugar de verdade", () => {
    ...
    rerender(<CameraControls target={[1, 3.15, 3]} />);
  ...
  it("descarta ao desmontar, para não vazar ouvinte no canvas", () => {
    const { unmount } = render(<CameraControls target={[1, 2, 3]} />);
