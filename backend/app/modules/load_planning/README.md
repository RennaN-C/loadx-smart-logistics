...
- `CONFIRMADO`: OC12 calcula o volume individual e expande quantidades no núcleo puro.
- `CONFIRMADO`: existem primitivas isoladas para limites/classificação geométrica da OC16 e controle de peso da OC18.
- `PENDENTE DE DEFINIÇÃO`: OC13–OC17, OC19 e OC20 ainda dependem das regras totais registradas em `docs/11-riscos-pendencias.md` antes de formar uma engine.
- `DECISÃO NECESSÁRIA`: persistência, service e API não podem ser iniciados antes da decisão sobre volumes, da engine fisicamente completa e dos contratos de estado/transação.
